...
export const metadata: Metadata = { title: "Edit Portfolio Category" };

interface Props {
  params: Promise<{ id: string }>;
}

export default async function EditPortfolioCategoryPage({ params }: Props) {
  const { id } = await params;
  const category = await prisma.portfolioCategory.findUnique({ where: { id: parseInt(id) } });
